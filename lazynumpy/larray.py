...
class lndarray(np.ndarray):
	def __new__(cls, obj, _hashname=None):
		...
		if len(self.shape) > 2:
			warnings.warn("Should not be using lndarray with more than 2-D arrays (0, 1 and 2 will work fine)", stacklevel=3)
	...
	def ldot(self, other):
		...
		if len(self.shape) == 2 and len(other.shape) == 2 and isinstance(other, lndarray):
			...
			new._evals = self._evals * other._evals
			return new
		warnings.warn("Should not be using lndarray with ldot if both are not lndarray with dimensions <= 2", stacklevel=3)
